...
import { IEvent } from 'app/shared/model/chatbot/event.model';
...
export class EventService {
    private resourceUrl = SERVER_API_URL + 'chatbot/api/events';
    ...
    private convertDateFromClient(event: IEvent): IEvent {
        const copy: IEvent = Object.assign({}, event, {
            dateTime: event.dateTime != null && event.dateTime.isValid() ? event.dateTime.toJSON() : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((event: IEvent) => {
            event.dateTime = event.dateTime != null ? moment(event.dateTime) : null;
        });
        return res;
    }
}
