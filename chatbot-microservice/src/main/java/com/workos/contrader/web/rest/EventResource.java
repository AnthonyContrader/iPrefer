...
import com.workos.contrader.service.EventService;
import com.workos.contrader.web.rest.errors.BadRequestAlertException;
import com.workos.contrader.web.rest.util.HeaderUtil;
import com.workos.contrader.web.rest.util.PaginationUtil;
...
public class EventResource {
    ...
    @GetMapping("/reminder")
    @Timed
    public ArrayList<String> getNextEvents(Pageable pageable) {
        log.debug("REST request to get a page of Events");
        Page<EventDTO> page = eventService.findAll(pageable);
        ArrayList<String> nextEvents = new ArrayList<String>();
        
        for (EventDTO e: page) {
        	if (e.getDateTime().toLocalDate().equals(LocalDate.now())) {
        		if ((LocalTime.now().until(e.getDateTime().toLocalTime(), ChronoUnit.MINUTES) <= 10) 
        				&& (LocalTime.now().until(e.getDateTime().toLocalTime(), ChronoUnit.MINUTES) > 0)) {
        			
        			String message = "L'evento " + e.getEventName() + " avverrà tra "+ LocalTime.now().until(e.getDateTime().toLocalTime(), ChronoUnit.MINUTES) + " minuti";
        			nextEvents.add(message);
        		}
        	}	
        }
        if (nextEvents.isEmpty()) {
        	nextEvents.add("Non ci sono eventi in scadenza");
        	return nextEvents;
        	
        } else
        	return nextEvents;
        
        //HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/reminder");
